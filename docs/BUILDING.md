...
The LibreSSL overlay port also carries `msvc-arm64-bn-ct-ne-zero.patch`: the workaround of
LibreSSL pull/1355 for the Visual Studio 2022 ARM64 optimizer bug that broke every EC, RSA and DH
operation of the ARM64 build ([libressl/portable#1403](https://github.com/libressl/portable/issues/1403));
drop it when a LibreSSL release includes that workaround, or when the ARM64 libraries are built
with Visual Studio 2026 v18.8.2 or later, and let the ARM64 jobs of the CI confirm the result.
