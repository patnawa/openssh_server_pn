...
try {
    ...
    try {
        foreach ($arch in $Architecture) {
            ...
            try {
                # Install into vcpkg_installed\<triplet>\ so that the layout is vcpkg_installed\<triplet>\<triplet>\,
                # the one MSBuild's vcpkg integration uses (VcpkgInstalledDir) and the check below expects. Without
                # it vcpkg uses its flat default (vcpkg_installed\<triplet>\) and a fresh tree always fails the check.
                & $vcpkgExe install `
                    --triplet $triplet `
                    "--x-install-root=$(Join-Path $installedDir $triplet)" `
