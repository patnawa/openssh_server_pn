...
{
    # The source root is the folder that holds version.h and contrib\win32\openssh. Looking for these
    # instead of .git works both when the tree is its own git repository and when it is a folder
    # (for example src\) inside a larger project repository.
    $start = $currentDir = (Get-Item -Path $PSScriptRoot)
    while ($null -ne $currentDir)
    {
        if ((Test-Path -Path (Join-Path -Path $currentDir.FullName -ChildPath 'version.h')) -and
            (Test-Path -Path (Join-Path -Path $currentDir.FullName -ChildPath 'contrib\win32\openssh')))
        {
            return $currentDir
        }
